...
export function LoginForm({ agenciaSlug }: { agenciaSlug?: string }) {
  const { login } = useAuth();
  ...
  useEffect(() => {
    api
      .get<Branding>(agenciaSlug ? `/branding/publico?slug=${encodeURIComponent(agenciaSlug)}` : "/branding/publico")
      .then(setBranding)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await login(email, password, agenciaSlug);
